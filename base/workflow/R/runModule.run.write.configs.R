...
#' @importFrom dplyr %>%
#' @export
runModule.run.write.configs <- function(settings, overwrite = TRUE, input_design = NULL, use.existing.samples = TRUE) {
  ...
  if (PEcAn.settings::is.MultiSettings(settings)) {
    ...
    return(PEcAn.settings::papply(settings, runModule.run.write.configs, overwrite = FALSE,input_design=input_design,use.existing.samples=use.existing.samples))
